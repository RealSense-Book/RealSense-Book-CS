...
namespace RealSenseSample
{
    ...
    public partial class MainWindow : Window
    {
        ...
        TextBlock[] tb;         //心拍の値を表示するTextBlockを用意する
        ...
        private void Initialize()
        {
            try
            {
                ...
                //追加：心拍数表示のための初期化
                tb = new TextBlock[PULSE_MAXFACES];
                for (int i = 0; i < PULSE_MAXFACES; i++)
                {
                    tb[i] = new TextBlock();
                    tb[i].Width = 200;
                    tb[i].Height = 27;
                    tb[i].Foreground = new SolidColorBrush(Colors.Red);
                    tb[i].FontSize = 24;
                    CanvasPoint.Children.Add(tb[i]);
                }
        ...
        private void InitializeFace() {
            ...
            config.QueryPulse().Enable();    //追加：顔の心拍情報取得を可能にする
            config.QueryPulse().properties.maxTrackedFaces = PULSE_MAXFACES; //追加：心拍数を取得できる人数を設定する
            config.ApplyChanges();

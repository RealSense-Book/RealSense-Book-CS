...
namespace RealSenseSample
{
    ...
    public partial class MainWindow : Window
    {
        ...
        TextBlock[,] tb;         //特徴位置を表示するTextBlockを用意する
        const int DETECTION_MAXFACES = 2;    //顔を検出できる最大人数を設定
        const int LANDMARK_MAXFACES = 2;    //特徴位置を検出できる最大人数を設定
        ...
        private void Initialize()
        {
            try
            {
                ...
                //追加：ランドマーク表示のための初期化
                tb = new TextBlock[LANDMARK_MAXFACES,78];
                for (int i = 0; i < LANDMARK_MAXFACES;i++)
                {
                    for (int j = 0; j < 78; j++) {
        ...
        private void InitializeFace() {
            ...
            config.landmarks.isEnabled = true;                   //追加：顔のランドマーク情報取得を可能にする
            config.landmarks.maxTrackedFaces = LANDMARK_MAXFACES;    //追加：二人までのランドマークを取得可能に設定する
            config.ApplyChanges();
